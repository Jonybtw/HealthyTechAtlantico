...
export function RangeSlider({
  ...
  const labelIndex = labels
    ...
    : undefined;
  const activeLabel =
    labelIndex !== undefined ? labels?.[labelIndex] : undefined;
  const activeColor =
    colorStops?.[labelIndex ?? 0] ??
    "bg-gradient-to-r from-navy-600 via-navy-700 to-navy-900";

  return (
    <div className="space-y-3 rounded-[24px] border border-border/60 bg-background/60 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="min-w-[74px] rounded-[24px] border border-border/60 bg-card/90 px-3 py-2 text-right shadow-sm">
          <p className="text-xl font-black tabular-nums tracking-[-0.06em] text-foreground">
            {safeValue}
          </p>
          {activeLabel ? (
          ...
          {quickChoices.map((choice) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "min-h-11 rounded-[24px] border px-3 py-2 text-sm font-semibold transition-all duration-300",
                  active
      ...
      >
        ...
        <input
          ...
          aria-label={label}
          aria-valuetext={
            activeLabel ? `${safeValue} ${activeLabel}` : String(safeValue)
          }
      ...
      <div className="flex items-center justify-between text-tiny font-semibold uppercase tracking-[0.16em] text-muted-foreground">
      ...
      {labels ? (
        <div
          className={`grid gap-2 text-micro font-medium text-muted-foreground ${labels.length === 3 ? "grid-cols-3" : "grid-cols-5"}`}
        >
          {labels.map((item, index) => (
            <span
              ...
              className={cn(
                "text-center",
                index === labelIndex && "text-foreground",
              )}
            >
              {item}
            </span>
