...
import type { TestPreviewRow } from "@/app/api/tests/import/preview/route";

type Step = "upload" | "preview";

interface TestBulkImportModalProps {
  open: boolean;
  onClose: () => void;
...
function downloadErrorReport(rows: TestPreviewRow[]) {
  const errorRows = rows.filter((r) => r.status === "error");
  if (!errorRows.length) return;
  const headers = ["row", "student", "test", "value", "date", "error"];
  const lines = errorRows.map((r) =>
    [r.row, r.student, r.testId, r.value, r.date, r.errorMessage ?? ""].join(","),
  );
  ...
  const link = document.createElement("a");
  link.href = url;
  link.download = "test_import_errors.csv";
  document.body.appendChild(link);
  link.click();
  link.remove();
  setTimeout(() => URL.revokeObjectURL(url), 1000);
...
export function TestBulkImportModal({
  open,
  onClose,
  onSuccess,
}: TestBulkImportModalProps) {
  const t = useTranslations("testes");
  const [step, setStep] = useState<Step>("upload");
  const [isDragging, setIsDragging] = useState(false);
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [previewRows, setPreviewRows] = useState<TestPreviewRow[]>([]);
  ...
  const processFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const res = await fetch("/api/tests/import/preview", {
          method: "POST",
          body: formData,
        });
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && handleClose()}>
      <DialogContent
        className="flex max-h-[90vh] w-full max-w-[660px] flex-col gap-0 overflow-hidden bg-white p-0 dark:bg-navy-950"
        hideClose
      >
        {/* Header */}
...
function UploadStep({
  ...
  t: ReturnType<typeof useTranslations<"testes">>;
}) {
  return (
        ...
        <div className="max-h-[340px] overflow-y-auto">
          <table className="w-full text-sm">
            <thead className="sticky top-0 z-10 border-b border-border/70 bg-card">
              <tr>
                <th className="w-12 px-4 py-2.5 text-left text-tiny font-semibold uppercase tracking-[0.14em] text-muted-foreground">
                  {t("bulkImportColRow")}
                </th>
                <th className="px-4 py-2.5 text-left text-tiny font-semibold uppercase tracking-[0.14em] text-muted-foreground">
                  {t("bulkImportColStudent")}
                </th>
                <th className="w-32 px-4 py-2.5 text-left text-tiny font-semibold uppercase tracking-[0.14em] text-muted-foreground">
                  {t("bulkImportColTest")}
                </th>
                <th className="w-24 px-4 py-2.5 text-left text-tiny font-semibold uppercase tracking-[0.14em] text-muted-foreground">
                  {t("bulkImportColValue")}
                </th>
                <th className="w-28 px-4 py-2.5 text-left text-tiny font-semibold uppercase tracking-[0.14em] text-muted-foreground">
                  {t("bulkImportColDate")}
                </th>
                <th className="w-24 px-4 py-2.5 text-right text-tiny font-semibold uppercase tracking-[0.14em] text-muted-foreground">
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5">
                      <CellValue
                        ...
                        error={
                          isError &&
                          !!row.errorMessage?.toLowerCase().includes("aluno")
                        }
                      />
                    </td>
                    <td className="px-4 py-2.5">
                      <CellValue
                        ...
                        error={
                          isError &&
                          !!row.errorMessage?.toLowerCase().includes("testid")
                        }
                      />
                    </td>
                    <td className="px-4 py-2.5">
                      <CellValue
                        ...
                        error={
                          isError &&
                          !!row.errorMessage?.toLowerCase().includes("valor")
                        }
                      />
                    </td>
                    <td className="px-4 py-2.5">
                      <CellValue
                        ...
                        error={
                          isError &&
                          !!row.errorMessage?.toLowerCase().includes("data")
                        }
                      />
                    </td>
